...
var PDFViewerApplication = {
  ...
  zoomIn: function zoomIn(ticks) {
    ...
    window.notablePreScale = newScale;
    do {
    ...
    window.notableNewScale = newScale;
  },
          ...
          switch (_context7.prev = _context7.next) {
            ...
            case 3:
              ...
              return _context7.abrupt('return', loadingTask.promise.then(function (pdfDocument) {
                // window.documentLoadedPromise = new Promise( function (resolve, reject) {
                    _this2.load(pdfDocument);
                    // resolve();
                // }).then(window.annotationFunction);
...
var webViewerFileInputChange = void 0;
{
  webViewerFileInputChange = function webViewerFileInputChange(evt) {
    ...
    // Clear view
      window.NotableApplication.notableView.clear();
    ...
    if (_pdfjsLib.URL.createObjectURL && !_app_options.AppOptions.get('disableCreateObjectURL')) {
      ...
      if (file.name) {
        ...
        formData.append('blobUrl', _url.url);
...
var PDFPageView = function () {
  function PDFPageView(options) {
    ...
    div.style.marginLeft = Math.floor(210) + 'px';
    div.setAttribute('data-page-number', this.id);
    this.div = div;
    container.appendChild(div);

    this.LJEventsBinded = false;
  }
  ...
  _createClass(PDFPageView, [{
    ...
    value: function draw() {
      ...
      /**********************************************************************/
      let __id = this.id;
      window.appPromise.then(function() {
          let notableApplication = window.NotableWebApp.NotableApplication;
          let notableView = notableApplication.notableView;

          let commentsLayer = document.querySelector('#commentsLayer');

          let left = $(_this.div).offset().left + $(_this.div).width() + 5;
          $(commentsLayer).css('left', left);


          // notableView.pages.push(_this.div);
          if(!_this.LJEventsBinded) {
              $(_this.div).on('mouseenter', function(evt) {
                  let e = evt || window.event;
                  console.log('enter' + __id);
                  window.webProperties.PreviousC.x = e.clientX;
                  window.webProperties.PreviousC.y = e.clientY;

                  window.webProperties.CurPageNum = _this.id;
                  // Create Image
                  if(webProperties.EventMode === webProperties.EVENTS_MODE_ENUM.MOD_DRAW_IMG) {
                      window.notableCreateImage();
                  }
              });
              $(_this.div).on('mouseleave', function() {
                  console.log('leave' + __id);
                  if(webProperties.EventMode === webProperties.EVENTS_MODE_ENUM.MOD_DRAW_IMG) {
                      $(notableView.curImage.preDiv).remove();
                  }
                  // Remove Image
              });
              _this.LJEventsBinded = true;
          }

          // update scale
          let createAnnotationLayerBottom = function(oldScale, newScale) {
            for(let num = 0; num < notableView.annotationLayerBottoms.length; num++) {
              let bottom = notableView.annotationLayerBottoms[num];
              if(bottom.id === __id) {
                $(div).append(bottom.div);
                return;
              }
            }
            let notableAnnotationLayer = document.createElement('div');
            notableAnnotationLayer.classList.add('notableAnnotationLayer');
            notableAnnotationLayer.classList.add('notableAnnotationLayer-bottom');
            notableAnnotationLayer.setAttribute('id', 'notableAnnotationLayer'+__id+'-bottom');
            div.appendChild(notableAnnotationLayer);

            let bot = {
                id: __id,
                hlRect: [],
                div: notableAnnotationLayer
            };
            notableView.annotationLayerBottoms.push(bot);
            return bot;
          };
          createAnnotationLayerBottom(window.notablePreScale, window.notableNewScale);
          let createAnnotationLayerTop = function(oldScale, newScale) {
              for(let num = 0; num < notableView.annotationLayerTops.length; num++) {
                  let top = notableView.annotationLayerTops[num];
                  if(top.id === __id) {
                      $(div).append(top.div);
                      notableView.resizeElements(__id, oldScale, newScale);
                      return;
                  }
              }
              let notableAnnotationLayer = document.createElement('div');
              notableAnnotationLayer.classList.add('notableAnnotationLayer');
              notableAnnotationLayer.classList.add('notableAnnotationLayer-top');
              notableAnnotationLayer.setAttribute('id', 'notableAnnotationLayer'+__id+'-top');
              ...
              notableView.annotationLayerTops.push(top);
              return top;
          };
          createAnnotationLayerTop(window.notablePreScale, window.notableNewScale);

          let createSvgLayer = function(oldScale, newScale) {
              // alert('draw');
            // if(notableView.svgLayers.length === 0) return;
            for(let num = 0; num < notableView.svgLayers.length; num++) {
              let svg = notableView.svgLayers[num];
              if(svg.id === __id) {
                $(div).append(svg.div);
                svg.container = div;
                svg.resizeElements(oldScale, newScale);
                return;
              }
            }
              // alert(notableView.svgLayers.length);
            let svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
            svg.setAttribute('id', 'painting' + __id);
            svg.setAttribute('style',"top:0; left: 0; position:absolute; width: 100%; height: 100%");
            ...
            svg.classList.add('shape-layer');

            div.appendChild(svg);

            let svgLayer = new window._notable_svg_layer_.NotableSVGLayer({
                notableView: notableView,
                id: __id,
                div: svg,
            });
            notableView.svgLayers.push(svgLayer);
            notableView.svgEventsBind(svgLayer);
            notableView.currentSvgLayer = svgLayer;
            return svgLayer;
          };
          let svgLayer = createSvgLayer(window.notablePreScale, window.notableNewScale);

          let createDrawingSvgLayer = function(oldScale, newScale) {
              for(let num = 0; num < notableView.drawingSvgLayers.length; num++) {
                  let svg = notableView.drawingSvgLayers[num];
                  if(svg.id === __id) {
                      //
                      $(div).append(svg.div);
                      svg.container = div;
              ...
              let notableDrawingSVG = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
              notableDrawingSVG.setAttribute('id', 'painting' + __id + '-drawing');
              notableDrawingSVG.setAttribute('style',"top:0; left: 0; position:absolute; width: 100%; height: 100%");


              if(webProperties.ToolMode !== 'drawing') {
                notableDrawingSVG.classList.add('no-events');
              } else {
                  notableDrawingSVG.classList.remove('drawing-mode');
              }
              notableDrawingSVG.classList.add('drawing-layer');
              div.appendChild(notableDrawingSVG);

              let dsvgLayer = new window._notable_svg_layer_.NotableSVGLayer({
                  notableView: notableView,
                  id: __id,
                  div: notableDrawingSVG,
                  shapeSvg: svgLayer,
              });
              notableView.drawingSvgLayers.push(dsvgLayer);
              notableView.drawingSvgEventsBind(dsvgLayer);
              notableView.curDrawingSvgLayer = dsvgLayer;
              return dsvgLayer;
          };
          createDrawingSvgLayer(window.notablePreScale, window.notableNewScale);
      });
...
function _download(blobUrl, filename) {
  // TODO():
     ...
     * @Author Leijie
     */
  let page = document.querySelector('#page' + window.webProperties.CurPageNum);
  let modifiedContents = window.webProperties.getModifiedContents();
  let myUrl = blobUrl;
  let obj = {
          blobUrl: blobUrl,
          fileName: filename,
